...
const JobCard = ({ job, matchScore = 0, matchExplanation = '' }) => {
    ...
    >
      ...
      >
        <div className="bg-white rounded-2xl overflow-hidden">
          ...
          {/* Match Score Badge */}
          {matchScore > 0 && (
            <span className={`absolute top-4 left-4 ${getMatchColor(matchScore)} text-xs px-2 py-1 rounded-full font-bold border shadow-sm z-10`}>
              {matchScore}% Match
            </span>
          )}

          {/* Match Explanation Tooltip */}
          {matchExplanation && matchScore > 0 && (
            <div className="absolute top-12 left-4 bg-white border border-gray-200 rounded-lg p-2 shadow-lg z-20 max-w-xs text-xs text-gray-600 hidden group-hover:block">
              💡 {matchExplanation}
            </div>
          )}
          ...
          <div className="p-6 pb-3 flex justify-between items-start">
            ...
            <button
              ...
              className={`p-2 rounded-full text-xl ${isSaved ? "text-indigo-500" : "text-gray-400 hover:text-indigo-600"
                } transition`}
          ...
          <div className="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-between items-center">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className={`px-4 py-2 text-xs font-semibold rounded-md transition-all duration-300 shadow-sm
                  ${isApplied
                    ? "bg-green-100 text-green-700 border border-green-200 cursor-default"
                    : "text-white bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-blue-600 hover:to-indigo-600 hover:shadow-md"
                  }`}
              >
                {isApplied ? "Applied" : (job.externalUrl ? "Apply External ↗" : "Apply Now")}
              </button>
            </div>
          </div>
        </div>
