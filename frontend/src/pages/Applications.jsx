                ...
                <div className="flex flex-wrap gap-2">
                  {['all', 'accepted', 'pending', 'rejected'].map((status) => (
                    <motion.button
                      ...
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      className={`px-4 py-2 rounded-lg font-medium transition-all duration-200 ${selectedStatus === status
                        ...
                        >
                          ...
                          <div className="relative flex items-center justify-between p-4 bg-slate-700/30 border border-slate-600/30 rounded-lg hover:bg-slate-700/50 hover:border-slate-500/50 transition-all duration-300">
                            <div className="flex items-center space-x-4 flex-1 min-w-0">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-1 mt-2">
                                  <div className={`w-2 h-2 rounded-full ${job.status ? 'bg-blue-500' : 'bg-slate-500'}`}></div>
                                  <div className={`w-8 h-0.5 ${job.status === 'Accepted' || job.status === 'Rejected' ? 'bg-blue-500' : 'bg-slate-600'}`}></div>
                                  <div className={`w-2 h-2 rounded-full ${job.status === 'Accepted' ? 'bg-emerald-500' : job.status === 'Rejected' ? 'bg-red-500' : 'bg-slate-600'}`}></div>
                                  <span className="text-[10px] text-slate-500 ml-1">
                                    {job.status === 'Accepted' ? 'Offer' : job.status === 'Rejected' ? 'Closed' : 'In Review'}
                                  </span>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="flex-shrink-0 flex flex-col gap-2 items-end">
                              ...
                              {/* External Job Status Update */}
                              {job.jobId?.externalUrl && (
                                <div className="mt-2">
                                  <select
                                    className="text-xs bg-slate-800 text-slate-300 border border-slate-600 rounded px-2 py-1 outline-none focus:border-blue-500"
                                    value={job.status}
                                    onChange={async (e) => {
                                      const newStatus = e.target.value;
                                      try {
                                        const token = await getToken();
                                        const { data } = await fetch(`${backendUrl}/api/users/update-status`, {
                                          method: 'POST',
                                          headers: {
                                            'Content-Type': 'application/json',
                                            'Authorization': `Bearer ${token}`
                                          },
                                          body: JSON.stringify({ applicationId: job._id, status: newStatus })
                                        }).then(res => res.json());
