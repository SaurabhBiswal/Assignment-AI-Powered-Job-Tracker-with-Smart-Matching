...
export const registerCompany = async (req, res) => {
  ...
  try {
    ...
    if (companyExists) return res.json({ success: false, message: "Company Already Exists" });
...
export const getCompanyById = async (req, res) => {
  try {
    const { id } = req.params;
    if (id.length !== 24) return res.json({ success: false, message: "Invalid ID format" });
...
// ... baaki saare functions jo pehle the (getCompanyData, postJob, etc.) waise hi rakhna
...
export const postJob = async (req, res) => {
  const { title, description, location, salary, level, category, jobType, workMode, skills, externalUrl } = req.body;
